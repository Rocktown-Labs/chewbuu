...
- Fixed native startup crash on Hermes (no secure RNG): added `react-native-get-random-values` synchronously in `apps/native/polyfills.js` so `@tanstack/db` UUID generation works at import time; fixed dead `Stack.Screen` names (`date`/`review`) that caused layout-children warnings.
- Wired the Blocks API for React Native: new `apps/native/lib/blocks-client.ts` registers an `onRequest` middleware (re-exported `registerMiddleware` from `@chewbuu/aws-blocks`) that forwards the Better Auth session cookie from SecureStore, since `credentials: "include"` is a browser-only no-op on native and every authenticated Blocks method gates on `requireSession(headers)`; fails fast with a clear message when `EXPO_PUBLIC_BLOCKS_API_URL` is unset and warns when it points at localhost (unreachable from physical devices).
- Native Blocks auth now uses the documented Better Auth Expo pattern (`authClient.getCookie()` per https://better-auth.com/docs/integrations/expo) instead of hand-reading the SecureStore cookie jar.
- Web onboarding Stripe Identity verification now opens in a new tab (popup-blocker-safe) instead of navigating the whole page away: polls verification status every 3s, refreshes on window focus, handles the `?verification=complete` return_url on mount, and falls back to same-tab navigation only when popups are blocked.
- Fixed Sync app startup in the same PR: added the missing `apps/sync/.env` (+ tracked `.env.example`, gitignored like native) so `@chewbuu/env` validation no longer throws at import; brought sync `auth-client` to parity (`/api/auth` baseURL, username support); added `chewbuu-sync://` to server `trustedOrigins`; added the RNG polyfill to sync polyfills; extracted shared `createNativeBlocksAuthMiddleware` in `@chewbuu/aws-blocks` (both apps forward the Better Auth session cookie on Blocks calls) with `apps/sync/lib/blocks-client.ts` wired into `venue-api`. Closes #136.
- Fixed sync bundler errors: removed the web stylesheet import from `apps/sync/global.css` (its shadcn `@property`/keyframes/complex `calc()` broke Uniwind and threw `SyntaxError` at bundle time) replacing it with Uniwind-native burgundy brand tokens; added a shared babel plugin (`scripts/babel-strip-node-fs-dynamic-import.cjs`, wired via `babel.config.cjs` in both Expo apps) that rewrites `@aws-blocks/core`'s dynamic `import("node:fs")` — dead code on native, but unparseable by Hermes — so `expo export`/EAS production bundles succeed (verified headless for both apps); applied pending Expo SDK 57 patch updates (`npx expo install --fix`) in both apps.
- Local device development: `dev:blocks` web frontend now binds LAN (`--host 0.0.0.0`) instead of loopback so Expo Go on a physical device can reach `:3001` auth alongside the `:3000` Blocks front-door.
- Auth cookies are environment-aware: production keeps `SameSite=None; Secure`, but local dev now uses `Lax` without `Secure`. Browsers silently drop `Secure` cookies over plain HTTP (LAN IPs, Safari), which discarded the session right after sign-up/sign-in and bounced users back to the login form.
- Local auth robustness: added `@aws-blocks/bb-data` as a root dev dependency so the dev server's `npx --no-install bb-data migrate` resolves instead of 404ing from the public registry; auth emails log their action links to the server console in non-production when `RESEND_API_KEY` is unset (verification/password-reset mail is otherwise silently dropped in local dev); renamed route-colocated vitest files with a `-` prefix so TanStack Router stops warning about them at startup.
- `bun run dev` now starts the backend too (closes #137): `dev`/`dev:portless` package tasks for `@chewbuu/db` (Postgres container) and `@chewbuu/aws-blocks`, composed via `with`; `server.ts` accepts `--no-frontend`/`--no-postgres` so turbo owns the UI and database with no duplicate spawns (standalone `dev:blocks` unchanged); web vite binds LAN with `strictPort`; Expo apps pinned to distinct Metro ports (`:8081` dating, `:8082` sync).
